    ...
    for i, nData in enumerate(bytes_data):
        if i==12:
            ...
            with err4:
                ...
                st.write(msg_value)
                msg_value = ("🔴" if (nData & 0x04) > 0 else "🟢") + "副柜天车夹盒电机压紧超时"
                st.write(msg_value)
            ...
            with err4:
                ...
                st.write(msg_value)
                msg_value = ("🔴" if (nData & 0x40) > 0 else "🟢") + "读flash副柜坐标失败"
                st.write(msg_value)
                msg_value = ("🔴" if (nData & 0x80) > 0 else "🟢") + "写flash副柜坐标失败"
                st.write(msg_value)
        elif i==49: #30~52
            with err1:
            ...
            with sig3:
                ...
                st.write(msg_value)
                msg_value = ("⚫" if (nData & 0x04) > 0 else "🟢") + "微波门上限(开)"
                st.write(msg_value)
                msg_value = ("⚫" if (nData & 0x08) > 0 else "🟢") + "微波门下限(关)"
                st.write(msg_value)
                msg_value = ("⚫" if (nData & 0x10) > 0 else "🟢") + "预留"
                st.write(msg_value)
                msg_value = ("⚫" if (nData & 0x20) > 0 else "🟢") + "预留"
